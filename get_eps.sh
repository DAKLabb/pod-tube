...
jq -c '.[]' $1 | while IFS= read -r item; do
    ...
    if [ -f "$filename" ]; then
      ...
      python update_feed.py $dir/feed.rss --title "$title" --url "https://github.com/DAKLabb/pod-tube/raw/refs/heads/main/$filename" --bytes $filesize --guid "$filename" --timestamp $timestamp --thumbnail $thumbnail --link $url --description "$description"
    fi
